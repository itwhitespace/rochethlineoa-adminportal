  ...
  const fetchData = async (
    ...
    try {
      const [membersRes, contentRes] = await Promise.all([
        DashboardService.getMembersList(searchStr, filterStr, pageNum, limit, 'Pending Register', occFilter, specFilter),
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-zinc-100 pb-5 dark:border-zinc-900">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-white">
            Visitors (Pending Register)
          </h2>
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
            List of LINE OA visitors who have not yet completed registration.
          </p>
        </div>
        <button
          ...
          <table className="w-full text-left text-sm text-zinc-500 dark:text-zinc-400">
            <thead className="bg-zinc-50/50 text-xs font-semibold uppercase tracking-wider text-zinc-400 border-b border-zinc-100 dark:bg-zinc-900/50 dark:border-zinc-800">
              <tr>
                ...
                <th className="px-6 py-4">First Session Join</th>
                  ...
                  <tr key={member.user_id} className="hover:bg-zinc-50/55 dark:hover:bg-zinc-900/40 transition-colors">
                    ...
                    <td className="px-6 py-4 text-center">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 rounded-lg border border-zinc-200 bg-white px-2.5 py-1.5 text-xs font-semibold text-zinc-700 shadow-xs hover:bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-300 dark:hover:bg-zinc-800"
                      >
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-sm text-zinc-400">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <span>No visitors found matching current search/filters</span>
                    </div>
                  </td>
                </tr>
              )}
        ...
        {!loading && totalItems > 0 && (
          <div className="flex items-center justify-between border-t border-zinc-100 bg-zinc-50/50 px-6 py-4 dark:border-zinc-800 dark:bg-zinc-900/30 text-xs font-medium text-zinc-500">
            <div>
              ...
              <span className="text-zinc-900 dark:text-white font-semibold">{totalItems}</span> Visitors
            </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative flex w-full max-w-2xl flex-col rounded-2xl border border-zinc-200 bg-white p-6 shadow-2xl dark:border-zinc-900 dark:bg-zinc-950 animate-fade-in-up max-h-[90vh] overflow-y-auto z-10">
            ...
            <div className="flex items-center justify-between border-b border-zinc-100 pb-4 dark:border-zinc-900">
              <h3 className="text-lg font-bold text-zinc-900 dark:text-white">Visitor Profile Details</h3>
              <button 
                onClick={() => setIsModalOpen(false)}
            ...
            <div className="flex flex-col items-center gap-4 py-6 border-b border-zinc-100 dark:border-zinc-900">
              <div className="relative flex h-24 w-24 items-center justify-center rounded-full bg-zinc-100 dark:bg-zinc-800 border-2 border-brand-blue/30 overflow-hidden shadow-sm">
              ...
              <div className="text-center">
                ...
                <span className="inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-semibold border bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/20 dark:text-amber-400 dark:border-amber-900/50 mt-2">
              ...
              <div className="space-y-3">
                ...
                <div className="rounded-lg bg-zinc-50 p-3.5 dark:bg-zinc-900/50 space-y-2">
                  ...
                  <div className="flex justify-between text-xs">
                    <span className="text-zinc-500 font-medium">First Join Date</span>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="flex items-start gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-bold text-zinc-850 dark:text-white">{selectedUser.occupation || 'Not Registered'}</p>
                      </div>
                    </div>
                    <div className="flex items-start gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-bold text-zinc-850 dark:text-white">{selectedUser.specialty || 'Not Registered'}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-4 pt-2 border-t border-zinc-100 dark:border-zinc-900">
                    <div className="flex items-start gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-bold text-zinc-850 dark:text-white">{selectedUser.organization || 'Not Registered'}</p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="flex items-start gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-bold text-zinc-850 dark:text-white truncate max-w-32" title={selectedUser.email || ''}>
                          {selectedUser.email || 'Not Registered'}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-start gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-bold text-zinc-850 dark:text-white">{selectedUser.phone || 'Not Registered'}</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div className="flex items-center justify-between p-2.5 rounded-lg border border-zinc-250/50 dark:border-zinc-800 bg-white dark:bg-zinc-950">
                    ...
                    <span className={`h-2 w-2 rounded-full ${selectedUser.occupation ? 'bg-emerald-500' : 'bg-zinc-300'}`} />
                  </div>
                  <div className="flex items-center justify-between p-2.5 rounded-lg border border-zinc-250/50 dark:border-zinc-800 bg-white dark:bg-zinc-950">
